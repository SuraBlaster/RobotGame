...
DefaultLitShader::DefaultLitShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
...
// 描画開始
void DefaultLitShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
...
// 描画
void DefaultLitShader::Draw(ID3D11DeviceContext* dc, const Model* model)
{
	...
	for (const ModelResource::Mesh& mesh : resource->GetMeshes())
	{
		...
		for (const ModelResource::Subset& subset : mesh.subsets)
		{
			...
			dc->PSSetShaderResources(1, 1, subset.material->SRV_Normal.GetAddressOf());
			dc->PSSetSamplers(0, 1, samplerState.GetAddressOf());
...
// 描画終了
void DefaultLitShader::End(ID3D11DeviceContext* dc)
{
